...
//? slice 사용 풀이
let str = "오늘은 2022년 08월 20일 입니다."

let str1 = str.slice(4, 17) // 문자열 슬라이싱 후 str1 선언
console.log(str1);
...
//! Q3. 
// 오늘은 2022년 8월 20일 입니다. 오늘에 해당되는 분기를 구하는 방법을 기술하세요. 123 / 456/ 789/ 101112

// 자바스크립트에서는 월의 시작이 0, 8월은 7로 표시
function getQuarter(month) {
  // Math.ceil함수(올림처리)를 사용하여 분기 계산
...
for (let i = 0; i < arr.length; i++) {
  ...
  for (let j = 0; j < arr.length -i -1; j++) {
    ...
    // >> 큰 값들이 배열의 오른쪽으로 이동
    if (arr[j] > arr[j+1]) {
      // 두 요소의 위치를 교환하기 위해 임시 변수(temp) 사용
      let temp = arr[j];
      arr[j] = arr[j+1];
      arr[j+1] = temp;
    }
...
let uniqueFilterArr = combinedArr.filter((value, index, self) => {
  ...
  // 특정 요소나 문자열의 첫번째 위치를 찾는데 사용하는 메소드
